...
def key(event):
    ...
    def Action(action, IdList):
        for idNumber in IdList:
            if action == "往前":
                dronelist[idNumber].set_velocity_body(id, gnd_speed, 0, 0)
            elif action == "往後":
                dronelist[idNumber].set_velocity_body(id, -gnd_speed, 0, 0)
            elif action == "往左":
                dronelist[idNumber].set_velocity_body(id, 0, -gnd_speed, 0)
            elif action == "往右":
                dronelist[idNumber].set_velocity_body(id, 0, gnd_speed, 0)
            elif action == "Yaw往左":
                dronelist[idNumber].condition_yaw(id, 10, True, -1)
            elif action == "Yaw往右":
                dronelist[idNumber].condition_yaw(id, 10, True, 1)
            elif action == "ChangMode":
                if dronelist[idNumber].current_mode == 9:
                    dronelist[idNumber].mode(id, 4)
                    dronelist[idNumber].current_mode = 4
                else:
                    dronelist[idNumber].mode(id, 9)
                    dronelist[idNumber].current_mode = 9
            elif action == "往上":
                dronelist[idNumber].set_velocity_body(id, 0, 0, -gnd_speed)
            elif action == "往下":
                dronelist[idNumber].set_velocity_body(id, 0, 0, gnd_speed)
            else:
    ...
    if event.char == event.keysym:  # -- standard keys
        if event.keysym == "w" or event.keysym == "W":
            ...
            dronelist[com_box.current()].servo(
                id, 9, dronelist[com_box.current()].servo_pwm
            )
        elif event.keysym == "2":
            dronelist[com_box.current()].servo_pwm -= 30
            ...
            for i in range(index_size):
                ...
                dronelist[com_box.current()].goto(
                ...
                if yaw_diff > 20 or yaw_diff < -20:
                    if yaw_diff > 0:
                        dronelist[com_box.current()].condition_yaw(
                            id, df.iloc[i][22], False, -1
                        )
                    else:
                        dronelist[com_box.current()].condition_yaw(
                            id, df.iloc[i][22], False, 1
        ...
        if event.keysym == "Up":
            Action("往上", [com_box.current(), 20])  # 20是Airsim
            labellist[6].config(bg="firebrick4")
        elif event.keysym == "Down":
            Action("往下", [com_box.current(), 20])  # 20是Airsim
...
takeoff_button = tk.Button(
    root,
    text="Takeoff",
    command=lambda: run_command_non_blocking(takeoff_button_callback),
)
connect_button1 = tk.Button(
    root,
...
def show_servo():
    servo_label.config(
        text="Servo angle : "
        + str(int(dronelist[com_box.current()].servo_pwm * 180 / 2000 - 135))
    )
